	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		if (request.getParameter("bookID") != "") {
			...
			if (bookTable != null) {
				...
				out.print("<script>alert('There is no such a book in library.');window.location='searchEditBook.jsp'</script>");
			}
		}else {
			PrintWriter out = response.getWriter();
			out.print("<script>alert('Please input book ID to edit it.');window.location='searchEditBook.jsp'</script>");
		}
	}
