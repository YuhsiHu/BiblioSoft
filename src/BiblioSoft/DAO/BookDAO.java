...
public class BookDAO {
	...
	public Collection searchByTitle(String title) {
		...
		if (title != "all" && title != "" && title != null) {
			sql = "select * from " + "bookinlib left join book on " + "bookinlib.isbn = book.isbn "
					+ "where book.book_name like " + "\'%" + title + "%\'";
		} else {
	...
	public Collection searchByPublisher(String publisher) {
		...
		if (publisher != "all" && publisher != "" && publisher != null) {
			sql = "select * from " + "bookinlib left join book on " + "bookinlib.isbn = book.isbn "
					+ "where book.publisher_name like " + "\'%" + publisher + "%\'";
		} else {
	 ...
	 * @return
	 */
	public ArrayList addBook(BookTable bookTable, int number) {
		...
		try {
			if (iscopyrs.next()) {
				...
				String addinlibsql = "insert into bookinlib (isbn,status,location) values (" + "\'"
						...
						+ bookTable.getLocation() + "\'" + ") returning book_id";
				
				ArrayList<Integer> idList=new ArrayList<Integer>();
				
				for (int i = 0; i < number; i++) {
					ResultSet rs=conn.executeQuery(addinlibsql);
					if(rs.next()) {
	 ...
	 * flag = 3,reserved
	 */
	public int deleteBookInLib(int id,String status1) {
		...
		String sql = "update bookinlib set status = "+"\'"+status1+"\'"+" where book_id = " + String.valueOf(id);
